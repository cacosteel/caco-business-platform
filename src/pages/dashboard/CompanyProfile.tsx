...
const activityLabels: Record<ActivityType, string> = {
  phone_call: "Phone call", email: "Email", meeting: "Meeting", message: "WhatsApp / message", follow_up: "Follow-up", note: "Note",
};
...
export default function CompanyProfile() {
  ...
  async function load() {
    try {
      const [companyData, contactData, activityData] = await Promise.all([getCompany(id), getCompanyContacts(id), getCompanyActivities(id)]);
      setCompany(companyData); setContacts(contactData); setActivities(activityData);
  ...
  return <Stack gap="lg">
    ...
    <Paper withBorder p="md"><Title order={3}>Activities</Title><Stack mt="md"><Select label="Activity type" value={activityType} onChange={(value) => setActivityType((value as ActivityType) || "phone_call")} data={ACTIVITY_TYPES.map((type) => ({ value: type, label: activityLabels[type] }))} /><Select label="Contact (optional)" clearable value={contactId} onChange={(value) => setContactId(typeof value === "string" ? value : null)} data={contacts.map((contact) => ({ value: contact.id, label: `${contact.first_name} ${contact.last_name}` }))} /><Textarea label="General context" required value={context} onChange={(event) => setContext(event.currentTarget.value)} /><Button onClick={() => void addActivity()} loading={saving}>Record activity</Button></Stack><Divider my="lg" />{activities.length === 0 ? <Text c="dimmed">No activities recorded.</Text> : activities.map((activity) => <div key={activity.id}><Text fw={600}>{activityLabels[activity.activity_type]}</Text><Text size="sm" c="dimmed">{new Date(activity.occurred_at).toLocaleString()}</Text><Text>{activity.context}</Text><Divider my="sm" /></div>)}</Paper>
