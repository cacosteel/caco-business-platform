...
export default function GmailSettings() {
  ...
  return <Paper maw={680} withBorder p="md"><Stack><div><Title order={1}>Google Workspace</Title><Text c="dimmed">Connect the CACO sending mailbox securely through Google OAuth.</Text></div>{status ? <Text>Connected sender: <strong>{status.sender_email}</strong></Text> : <Text c="dimmed">No Gmail mailbox is connected yet.</Text>}<Button onClick={() => void connect()} loading={saving}>{status ? "Reconnect Google Workspace" : "Connect Google Workspace"}</Button></Stack></Paper>;
}
