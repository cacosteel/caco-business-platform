...
export default function PlatformSettings() {
  ...
  return <Paper maw={720} withBorder p="md"><Stack gap="md"><div><Title order={1}>Platform Profile</Title><Text c="dimmed">Shared organisation details for the CACO platform. These will be available for future document and website features.</Text></div><TextInput label="Organisation name" value={settings.organisation_name} onChange={(event) => update("organisation_name", event.currentTarget.value)} required /><TextInput label="Legal company name" value={settings.legal_name ?? ""} onChange={(event) => update("legal_name", event.currentTarget.value)} /><TextInput label="Email" type="email" value={settings.email ?? ""} onChange={(event) => update("email", event.currentTarget.value)} /><TextInput label="Phone" value={settings.phone ?? ""} onChange={(event) => update("phone", event.currentTarget.value)} /><Textarea label="Formal address" value={settings.address ?? ""} onChange={(event) => update("address", event.currentTarget.value)} minRows={3} /><TextInput label="Country" value={settings.country ?? ""} onChange={(event) => update("country", event.currentTarget.value)} /><TextInput label="Website" placeholder="https://www.cacosteel.com" value={settings.website ?? ""} onChange={(event) => update("website", event.currentTarget.value)} /><Button onClick={() => void save()} loading={saving}>Save platform profile</Button></Stack></Paper>;
}
